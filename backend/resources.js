...
router.get('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    let queryText = `
      SELECT r.*,
        COALESCE(
          ...
          ) FILTER (WHERE b.id IS NOT NULL AND b.start_time::date = $1::date),
          '[]'
        ) as today_bookings
      FROM resources r
    ...
    if (type) {
      params.push(type);
      queryText += ' AND r.type = $' + params.length + '::resource_type';
    }
    ...
    if (date && start && end) {
      ...
      queryText += ' AND r.id NOT IN (SELECT resource_id FROM bookings WHERE status = ' + "'" + 'confirmed' + "'" + ' AND tstzrange(start_time, end_time, ' + "'[)'" + ') && tstzrange($' + (params.length-1) + '::timestamptz, $' + params.length + '::timestamptz, ' + "'[)'" + '))';
    }

    queryText += ' GROUP BY r.id ORDER BY r.type, r.name';
