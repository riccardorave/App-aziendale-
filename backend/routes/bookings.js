...
const { sendBookingConfirmEmail, sendCancellationNotifyEmail, sendRecurringBookingConfirmEmail } = require('../middleware/mailer');
...
router.post('/', authenticate, [
  ...
  const { resource_id, title, notes, start_time, end_time, recurring, weeks } = req.body;
  ...
  // Controlla risorsa
  const resCheck = await db.query('SELECT id, name FROM resources WHERE id=$1 AND is_active=true', [resource_id]);
  if (!resCheck.rows.length) return res.status(404).json({ error: 'Risorsa non trovata' });

  const totalWeeks = (recurring && weeks) ? Math.min(parseInt(weeks), 4) : 1;
  const recurrence_id = totalWeeks > 1 ? require('crypto').randomUUID() : null;
  ...
  for (let w = 0; w < totalWeeks; w++) {
    ...
    try {
      ...
      const conflict = await client.query(`
        ...
        FOR UPDATE
      `, [resource_id, s.toISOString(), e.toISOString()]);

      if (conflict.rows.length) {
        await client.query('ROLLBACK');
        skipped.push(s.toLocaleDateString('it-IT'));
        continue;
      }

      const { rows } = await client.query(`
        INSERT INTO bookings (user_id, resource_id, title, notes, start_time, end_time, recurrence_id)
        VALUES ($1,$2,$3,$4,$5,$6,$7)
        RETURNING *
      `, [req.user.id, resource_id, title, notes, s.toISOString(), e.toISOString(), recurrence_id]);

      await client.query('COMMIT');

      const full = await db.query(`
        SELECT b.*, u.name as user_name, u.email as user_email, u.avatar_color,
          r.name as resource_name, r.type as resource_type, r.location as resource_location
        FROM bookings b
        JOIN users u ON u.id = b.user_id
        JOIN resources r ON r.id = b.resource_id
      ...
      logActivity({
        ...
        details: `Prenotato "${bookingData.resource_name}" — ${bookingData.title} — ${new Date(bookingData.start_time).toLocaleString('it-IT')}${recurrence_id ? ' (ricorrente)' : ''}`,
        ip: req.ip,
      });
  ...
  if (!created.length) {
    return res.status(409).json({ error: 'Nessuna prenotazione creata: tutti gli slot sono occupati', skipped });
  }

  // Manda una sola email con tutte le date
  try {
    const userResult = await db.query('SELECT email, name FROM users WHERE id=$1', [req.user.id]);
    if (userResult.rows.length) {
      if (created.length === 1) {
        await sendBookingConfirmEmail(userResult.rows[0].email, userResult.rows[0].name, created[0]);
      } else {
        await sendRecurringBookingConfirmEmail(userResult.rows[0].email, userResult.rows[0].name, created);
      }
    }
  } catch (emailErr) {
  ...
  res.status(201).json({
    created,
    skipped,
    message: `${created.length} prenotazione/i creata/e${skipped.length ? `, ${skipped.length} saltata/e per conflitto` : ''}`,
  });
});
...
// Notifica utenti con prenotazioni sulla stessa risorsa nelle prossime 24 ore
try {
  const nearbyBookings = await db.query(`
  ...
  `, [booking.resource_id, booking.user_id]);

  for (const nb of nearbyBookings.rows) {
    await sendCancellationNotifyEmail(
      nb.user_email,
      nb.user_name,
      fb,
      req.user.email
  ...
  console.error('Errore invio notifica cancellazione:', notifyErr.message);
}
logActivity({
  userId: req.user.id,
  userName: req.user.name || fb.user_name,
  userEmail: req.user.email || fb.user_email,
  action: 'BOOKING_CANCELLED',
  entityType: 'booking',
  entityId: req.params.id,
  details: `Cancellata prenotazione "${fb.resource_name}" — ${fb.title}${req.user.id !== fb.user_id ? ' (cancellata da admin)' : ''}`,
  ip: req.ip,
});
